...
TeX-Net大概有一百万可训练参数，使用GPU NVIDIA RTX A6000 48 GB需要42毫秒。
...
>
> ​	其次，由于TeX简并性，必须指定数学结构$X_{\alpha v}=\sum_{\beta ≠\alpha} V_{\alpha \beta}S_{\alpha \beta}$以确保逆映射的唯一性，因此必须学习热照明因子V而不是纹理X。也就是说，TeX-Net不能够端到端地进行训练。
